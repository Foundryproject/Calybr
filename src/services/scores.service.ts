...
const getQuickTip = (data: DriverScoreData): string => {
  const tips: Record<number, string> = {
    [data.speeding_score]: "Watch your speed! Staying within limits improves safety and your score.",
    [data.hard_brakes_score]: "Anticipate stops early to avoid hard braking and improve comfort.",
    [data.phone_distraction_score]: "Put your phone away while driving. Use hands-free if needed.",
    [data.cornering_score]: "Take turns smoothly by reducing speed before the corner.",
    [data.night_driving_score]: "Drive slower at night and use high beams when appropriate.",
    [data.highway_score]: "Maintain a 3-second following distance on highways.",
  };

  const lowestScore = Math.min(...Object.keys(tips).map(Number));
  return tips[lowestScore] || "Great driving! Keep up the consistent safe habits.";
};
